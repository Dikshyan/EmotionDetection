...
while True:
    ...
    try:
        for i, roi in enumerate(face_rois):
            result = DeepFace.analyze(img_path=roi, actions=['emotion'], enforce_detection=True, detector_backend='retinaface')
            if isinstance(result, list) and len(result) > 0:
                ...
                if confidence > 50:
                    dominant_emotions[i] = dominant_emotion
                else:
                    dominant_emotions[i] = "Uncertain"
                
                emotion_history.append(dominant_emotions[i])
    except Exception as e:
        print(f"DeepFace analysis error: {str(e)}")
    
    most_common_emotion = max(set(emotion_history), key=emotion_history.count) if emotion_history else "Neutral"
